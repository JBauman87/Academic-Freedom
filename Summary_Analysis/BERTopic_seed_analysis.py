...
def bertopic(documents: list[str], embeddings:np.ndarray, seed: int, min_cluster_size: int,  n_neighbors: int,n_components: int):
    ...
    vectorizer_model = CountVectorizer(
        ...
        ngram_range=(1, 2),
        min_df=1 #word must appear in X documents, set manually
    )
...
def optimize_parameters(
    ...
    for min_size, neighbors in parameter_grid:
        ...
        print(
            f"cluster={min_size}, "
            f"neighbors={neighbors}, "
            f"n_topics={run_result['n_topics']}"
        )
...
if __name__ == "__main__":
    ...
    if args.mode == "optimize":
        ...
        OPTIMIZATION_DF = optimize_parameters(
            DOCUMENTS,
            EMBEDDINGS,
            min_cluster_sizes=[3, 6, 9, 12, 15], #CHANGE HERE
            n_neighbors_values=[4, 6, 8, 10], #CHANGE HERE
            n_components=5,
            seed=42,
        )
